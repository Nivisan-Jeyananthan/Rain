package ch.nivisan.rain.game.graphics;
...
public class Sprite {
	...
	/**
	 * Create a sprite from an existing spritesheet, where the sprite is the entire
	 * size
	 *
	 * @param size
	 * @param x
	...
	public static Sprite rotateSprite(Sprite sprite, float angle) {
		return new Sprite(rotate(sprite.pixels, sprite.getWidth(), sprite.getHeight(), angle), sprite.getWidth(),
				sprite.getHeight());
	}

	/**
	 * Rotation is done clockwise instead of according to unit of circle
	 * (counterclockwise). Therefor units are in negative instead of positive
	 *
	 * @param pixels
	 * @param spriteWidth
	 * @param spriteHeight
	...
	/**
	 * Calculates where the new x positions should be. Negation of sin due to clock
	 * wise rotation
	 *
			...
			for (int spriteX = 0; spriteX < sheet.getWidth() / spriteWidth; spriteX++) {
				...
				for (int y = 0; y < spriteHeight; y++) {
					for (int x = 0; x < spriteWidth; x++) {
						...
						spritePixels[x + y * spriteWidth] = sheet.getPixels()[originalPixelX
								+ originalPixelY * sheet.getWidth()];
					}
				}
