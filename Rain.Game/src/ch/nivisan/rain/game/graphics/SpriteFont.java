...
public class SpriteFont {
	...
	public void render(String text, Screen screen) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (currentCharacter == 'g' || currentCharacter == 'y' || currentCharacter == 'q' || currentCharacter == 'p'
					|| currentCharacter == 'j' || currentCharacter == ',') {
				yOffset += 4;
			}
			var index = charactes.indexOf(currentCharacter);
			if (index == -1)
				continue;
			screen.renderSprite(x + xOffset, y + (currentLine * 20) + yOffset, fontCharacter[index], false, true,
